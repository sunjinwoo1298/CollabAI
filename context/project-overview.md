...
- Canvas state is a Yjs document (Y.Map of nodes, Y.Map of edges) — CRDT-based, so concurrent edits converge without a central lock.
- Clients connect to a room-scoped WebSocket server; updates are Yjs binary diffs, not full document state.
- WS servers are horizontally scaled behind Redis pub/sub — clients on different server instances still see each other's updates within the same room.
- Presence (cursors, active selections) is broadcast via Yjs Awareness, a separate ephemeral channel not persisted or subject to conflict resolution.
- The Yjs document is snapshotted periodically to Postgres/Blob; on load, the server sends the latest snapshot plus any updates since, rather than replaying full history.
- Local edits apply optimistically to the client's in-memory Yjs doc before the network round-trip completes.
