...
export const CollaboratorCursor = memo(function CollaboratorCursor({
  ...
  return (
    ...
    >
      {/* SVG Mouse Pointer */}
      <svg
        className="h-5 w-5 -rotate-90 drop-shadow-md"
        viewBox="0 0 24 24"
        fill={userColor}
        stroke="#0B0F19"
        strokeWidth="1.5"
      ...
      <div
        className="ml-3 -mt-1 flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-medium text-white shadow-lg backdrop-blur-sm whitespace-nowrap"
        style={{
      ...
      >
        {user.avatar ? (
          <img
            src={user.avatar}
            alt={userName}
            className="h-3.5 w-3.5 rounded-full border border-white/40 object-cover"
          />
        ) : null}
        ...
        {isThinking && (
          <span className="flex items-center gap-0.5 bg-black/30 px-1 py-0.5 rounded-full text-[9px]">
